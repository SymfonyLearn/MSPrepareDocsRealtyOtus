<?php

declare(strict_types=1);

namespace App\Entity\ValueObject;

use Doctrine\ORM\Mapping as ORM;

use InvalidArgumentException;

/**
 * Class Email
 * @package App\Model\ValueObject
 * @ORM\Embeddable
 */
class Email
{

    const MESSAGE_IS_EMPTY_VALUE = 'Пустое значение';
    const MESSAGE_IS_NOT_VALID_VALUE = 'Некорректная почта';

    /**
     * @var string $value
     * @ORM\Column(type="string", length=255, unique=true)
     */
    private string $value;

    /**
     * Email constructor.
     * @param string $value
     */
    public function __construct(string $value)
    {
        $this->setValue($value);
    }

    /**
     * @param string $value
     */
    private function setValue(string $value): void
    {
        $this->assertIsNotEmpty($value);
        $this->assertIsValid($value);
        $this->value = $value;
    }

    /**
     * @return string
     */
    public function value(): string
    {
        return $this->value;
    }

    /**
     * Сравнение 2-ух объектов
     * @param self $otherValue
     * @return bool
     */
    public function equalsTo(self $otherValue): bool
    {
        return $this->value() === $otherValue->value();
    }

    /**
     * Проверить, что значение состояние не пустое.
     * @param string $value
     */
    private function assertIsNotEmpty(string $value)
    {
        if (empty($value)) {
            throw new InvalidArgumentException(self::MESSAGE_IS_EMPTY_VALUE);
        }
    }

    /**
     * Проверить, что значение почты корректно.
     * @param string $value
     */
    private function assertIsValid($value)
    {
        if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
            throw new InvalidArgumentException(self::MESSAGE_IS_NOT_VALID_VALUE);
        }
    }
}
